...
router.put('/:id/like' , verify , async(req,res) => {
    const { id } = req.params
    const post = await Post.findById(id)
    if(!mongoose.Types.ObjectId.isValid(id)) return res.status(403).send('No post was found with the given id')
    try {
        if(!post?.likes?.includes(req.user.id)) {
            ...
            const decreaseLike = await Post.findByIdAndUpdate(id , {
                $pull : {
                    likes: req.user.id
                }
            } , {new : true})
